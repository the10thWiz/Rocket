...
//!     fails, but 5.19 occationally passes. The only difference between the two cases is that 5.20
//!     splits the messages up and sends them byte by byte, which reinforces the idea that this is
//!     a race, and the rate of transmission effects the outcome.
...
pub mod rocket_multiplex {
    ...
    //! create a longer message. Control messages should be marked as text and take the form:
    //!
    //! `S ACTION S (PARAM S)*`
    //!
