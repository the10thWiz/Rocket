...
| `sync_threads`* | `usize`           | Limit on threads to start for synchronous tasks. | `512`                   |
...
<small>* Note: the `workers`, `sync_threads`, and `shutdown.force` configuration
...
The `sync_threads` parameter sets an upper limit on the number of threads the
...
parameter, `sync_threads` cannot be reconfigured or be configured from sources
other than those provided by [`Config::figment()`]. Unlike `workers`, threads
corresponding to `sync_threads` are not always active and will exit if idling.
